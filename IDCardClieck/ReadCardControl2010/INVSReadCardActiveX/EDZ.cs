...
namespace GSFramework
{
    ...
    public class EDZ
    {
        ...
        public string NATION_Code
        {
            ...
            {
                _NATION_Code = value;
                if (lstMZ.ContainsKey(value))
                {
                    NATION_CName = lstMZ[value].ToString();
                }
                else if (lstMZ.ContainsValue(value) || lstMZ.ContainsValue(value+"族"))
                {
                    //NATION_CName = value;
                    foreach (KeyValuePair<string, string> kvp in lstMZ)
                    {
                        if (kvp.Value.Equals(value) || kvp.Value.Equals(value + "族"))
                        {
                            NATION_Code = kvp.Key;
                            break;
                        }
